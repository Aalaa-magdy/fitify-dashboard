...
const AllUsersProgressChart = () => {
  ...
  const chartData = useMemo(() => {
    ...
    progressData.forEach(item => {
      ...
      if (item.calcMethod === 'time') {
        exerciseMap[name].timeInSeconds += Number(item.timeInSeconds || 0);
      } else {
        exerciseMap[name].correctReps += Number(item.correctReps || 0);
        exerciseMap[name].incorrectReps += Number(item.incorrectReps || 0);
      }
    });

    return Object.values(exerciseMap).map(item => {
      const correct = Number(item.correctReps || 0);
      const incorrect = Number(item.incorrectReps || 0);
  ...
  const filteredData = useMemo(() => {
    return chartData
      .filter(item => (calcMethod === 'time' ? item.isTimeBased : !item.isTimeBased))
